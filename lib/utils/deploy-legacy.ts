...
const showPushProgress = async function (message: string) {
...
const uploadImage = async function (
	...
	const progressMessage = logger
		...
		.slice(0, -1);
	const progressBar = await showPushProgress(progressMessage);
...
export const deployLegacy = async function (
	...
	const { buildId } = await bufferImage(docker, imageName, bufferFile)
		.then(
			async (stream) =>
				await uploadImage(stream, token, username, url, appName, logger),
		)
		.finally(async () =>
			...
			(await import('fs')).promises.unlink(bufferFile).catch(() => undefined),
		);
